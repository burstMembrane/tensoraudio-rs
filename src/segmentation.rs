...
pub fn split_samples(
    ...
    // Calculate padding for last segment
    let last_start = (num_segments - 1) * step_size;
    let needed_samples = last_start + segment_samples;
    let mut padding = if needed_samples > total_samples {
        needed_samples - total_samples
    } else {
        0
...
mod tests {
    use crate::audio::{
        ...
        save_tensor, tensor_to_vec_f32, NoiseColor,
    };
    use crate::segmentation::{merge, merge_batch, merge_samples, split, split_samples};
    ...
    #[test]
    fn test_split_merge_samples() {
        let sample_rate = 44100;
        // Create 1 second of test audio (sine wave)
        let samples = load_tensor("testdata/test.wav", Device::Cpu);
        let (segments, padding) = split_samples(
            &samples,
            22050, // 0.5 second segments
            0.1,   // 100ms overlap
            "linear",
            sample_rate as u32,
        )
        ...
        let reconstructed =
            merge_samples(&segments, 22050, 0.1, padding, "linear", sample_rate as u32).unwrap();

        assert_eq!(samples.size(), reconstructed.size());

        // save the reconstructed audio
        let output_path = PathBuf::from("testdata/reconstructed_samples.wav");
        let _ = save_tensor(
            &reconstructed,
            output_path.to_str().unwrap(),
            sample_rate,
